
<template>
  <div class="container">
    <div class="header">
      <div class="header-left">
        <van-icon name="arrow-left" size="30" @click="goBack"/>
      </div>
      填写服务订单
    </div>

    <status-bar item="0" />

    <van-cell class="cell">
      <template #title>
        <van-image :src="companionData.service.serviceImg" width="25px" height="25px"/>
        <div class="server-name">{{ companionData.service.serviceName }}</div>
      </template>
      <template #default>
        <div class="service-text">服务内容</div>
      </template>
    </van-cell>

    <van-cell-group class="cell">
      <van-cell>
        <template #title>就诊医院</template>
        <template #default>
          <span @click="showHospital=true" >{{ form.hospital_name || '请选择就诊医院' }}</span>
          <van-icon name="arrow" />
        </template> 
      </van-cell>

      <van-cell>
        <template #title>就诊时间</template>
        <template #default>
          <span @click="showTime=true" >{{ currentDate || '请选择就诊时间' }}</span>
          <van-icon name="arrow" />
        </template>
      </van-cell>

      <van-cell>
        <template #title>陪护师</template>
        <template #default>
          <span @click="showCompanion=true" >{{ companionName || '请选择陪护师' }}</span>
          <van-icon name="arrow" />
        </template>
      </van-cell>

      <van-cell>
        <template #title>接送地址</template>
        <template #default>
          <van-field v-model="form.receiveAddress" placeholder="请填写接送地址" class="text" input-align="right">
          </van-field>
        </template>
      </van-cell>

      <van-cell>
        <template #title>联系电话</template>
        <template #default>
          <van-field v-model="form.tel" placeholder="请填写联系电话" class="text" input-align="right">
          </van-field>
        </template>
      </van-cell>

    </van-cell-group>

    <van-cell-group title="服务需求" class="cell">
      <van-field 
        v-model="form.demand" 
        placeholder="请简单描述您要就诊的科室" 
        style="height: 100px;" >
      </van-field>
    </van-cell-group>

    <van-button class="submit" @click="submit" type="primary" size="large">提交订单</van-button>
    
    <!-- //就诊医院弹出框 -->
    <van-popup
      v-model:show="showHospital"
      position="bottom"
      :style="{ height: '30%' }">
      <van-picker
        :columns="hospitalColumns"
        @confirm="hospitalConfirm"
        @cancel="showHospital=false"
      />
    </van-popup>

    <!-- 就诊时间 -->
     <van-popup
      v-model:show="showTime"
      position="bottom"
      :style="{ height: '30%' }">
      <van-date-picker
        title="选择日期"
        :min-date="minDate"
        @confirm="timeConfirm"
      />

    </van-popup>

    <!-- 陪护师 -->
     <van-popup
      v-model:show="showCompanion"
      position="bottom"
      :style="{ height: '30%' }">
      <van-picker
        :columns="companionColumns"
        @confirm="companionConfirm"
        @cancel="showCompanion=false"
      />
    </van-popup>

    <!-- 支付弹窗 -->
    <van-dialog v-model:show="showCode" :show-confirm-button="false" >
      <van-icon name="cross" @click="closeCode" class="close" />
      <div>微信支付</div>
      <van-image :src="codeImg" width="150" height="150"></van-image>
      <div>请使用本人微信扫描二维码</div>
    </van-dialog>

  </div>
</template>

<script setup>
import { useRouter } from 'vue-router'
import statusBar from '../../components/statusBar.vue'
import { ref, reactive, getCurrentInstance, onMounted, computed } from 'vue'
import { showNotify } from 'vant'
import Qrcode from 'qrcode'
 
const router = useRouter()
const { proxy } = getCurrentInstance()
const companionData = reactive({
  companion:[],
  hospitals:[],
  service:{}
})
onMounted( async () => {
  const res = await proxy.$api.h5Companion()
  const data = res.data.data
  Object.assign(companionData, data)
  console.log(companionData)
})

//顶部
const goBack = () => {
  router.go(-1)
}

//就诊医院
const form = reactive({})
const showHospital = ref(false)
const hospitalColumns =  computed(() => {
  return companionData.hospitals.map((item) => {
    return {text: item.name, value: item.id}
    
  })
})
const hospitalConfirm = (item) => {
  //console.log(item)
  form.hospital_name = item.selectedOptions[0].text
  form.hospital_id = item.selectedOptions[0].value
  showHospital.value = false
}

//就诊时间
const showTime = ref(false)
const currentDate = ref()
const minDate = ref(new Date())

const timeConfirm = (item) => {
  //console.log(item)
  const timeStr = item.selectedValues.join('-')
  currentDate.value = timeStr
  form.starttime = new Date(timeStr).getTime()
  console.log(form.starttime)
  showTime.value = false
}

//陪护师
const showCompanion = ref(false)
const companionName = ref()
const companionColumns =  computed(() => {
  return companionData.companion.map((item) => {
    return {text: item.name, value: item.id}
    
  })
})
const companionConfirm = (item) => {
  console.log(item)
  form.companion_id = item.selectedValues[0]
  companionName.value = item.selectedOptions[0].text
  showCompanion.value = false
}

//提交
const showCode = ref(false)
const codeImg = ref("")
const submit = async () => {
  const params = [
    'hospital_id',
    'hospital_name',
    'demand',
    'companion_id',
    'receiveAddress',
    'tel',
    'starttime'
  ]

  for(const i of params){
    if(!form[i]){
      showNotify({message: '请把每一项都填写'})
    }
  }
  const res = await proxy.$api.createOrder(form)
  //console.log(res)
  if(res.data.code === 10000){
    showCode.value = true
    Qrcode.toDataURL(res.data.data.wx_code).then(url => {
      codeImg.value = url
    })
  }
}

const closeCode = () => {
  showCode.value = false
  router.push('/order')
}
</script>

<style lang="less" scoped>
.container {
  background-color: #f0f0f0;
  height: 100vh;
}
  .header {
    background-color: #fff;
    padding: 10px 0;
    text-align: center;
    line-height: 30px;
    font-size: 15px;
    .header-left {
      float: left;
    }
  }

  .cell {
    width: 95%;
    margin: 5px auto;
    background-color: #fff;
    ::v-deep(.van-field__control) {
      color: var(--van-cell-value-color);
    }
    ::v-deep(.van-cell__title) {
    display: flex;
    align-items: center;
    }
    .server-name {
      margin-left: 10px;
    }
    
  }
  .service-text {
      background: url(data:image/png;base64,)
        no-repeat center center;
      background-size: 20px;
    }
  
  .submit {
    position: absolute;
    bottom: 0;
  }
  ::v-deep(.van-dialog__content) {
    text-align: center;
    padding: 20px;
    .close {
      position: absolute;
      left: 20px;
    }
  }

</style>