<template>
  <div class="container">
    ...
    <van-swipe class="my-swiper" style="height: 170;">
      <van-swipe-item v-for="item in homeData.slides" :key="item.id">
        <!-- <img :src="item.pic_image_url"> -->
        <van-image :src="item.pic_image_url" radius="5"></van-image>
      </van-swipe-item>
      
    </van-swipe>
    <van-row justify="space-around" >
      <van-col span="11" v-for="( item, index ) in homeData.nav2s" :key="item.id">
        <van-image :src="item.pic_image_url" @click="goOrderTo(index)"></van-image>
      </van-col>
    </van-row>
...
onMounted( async () => {
  const data = await proxy.$api.indexIndex()
  //console.log(res)
  if(data.data.code === 10000){
    Object.assign(homeData,data.data.data)
